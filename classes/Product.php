...
class ProductCore extends ObjectModel
{
	...
	public static function getKovanieOrOtherAttribute($id_attribute, $id_lang)
	{
		$return = false;
		$result = Db::getInstance()->executeS('
			SELECT al.`name`, ag.`public_name`
			...
			LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` ag ON (ag.`id_attribute_group` = a.`id_attribute_group`) AND ag.`id_lang` = '.$id_lang.'
			WHERE pac.`id_product_attribute` = '.(int)$id_attribute.' 
			');
		if($result[0]){
			$return = $result[0]["public_name"]." : ".$result[0]["name"];
			return $return;	
		}
		return $return;
	}
	...
	**/
		...
		$id_customer = 0, $use_customer_price = true, $id_cart = 0, $real_quantity = 0, $id_kovanie = null, $id_vzor = null, $id_dvere = null, $id_zarubna = null)
	{
		...
		if($id_kovanie != null || (int)$id_kovanie != 0){
			$sql = new DbQuery();
			...
			$sql->where('pa.`id_product_attribute` = '.(int)$id_kovanie);
			$kovaniePriceArray = Db::getInstance()->getRow($sql);
		}
		if($id_vzor != null || (int)$id_vzor != 0){
			$sql = new DbQuery();
			$sql->select('pa.`price`,pa.`ecotax`,sp.`price` AS `specific_price`');
			$sql->from('product_attribute', 'pa');
			$sql->leftJoin('specific_price', 'sp', 'pa.`id_product_attribute` = sp.`id_product_attribute`');
	...
	*/
	...
	public function getAttributesGroups($id_lang)
	{
		...
		$sql = 'SELECT ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, agl.`public_name` AS public_group_name,
					a.`id_attribute`, al.`name` AS attribute_name, a.`color` AS attribute_color, pa.`id_product_attribute`,pa.`upc`,
