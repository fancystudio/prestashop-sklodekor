	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dd id="cart_block_combination_of_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}_{$product.id_address_delivery|intval}_{if $product.id_kovanie}{$product.id_kovanie}{else}0{/if}_{if $product.id_vzor}{$product.id_vzor}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<a href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)}" title="{l s='Product detail' mod='blockcart'}">{$product.attributes_small}{if $product.kovanie_popis}, {$product.kovanie_popis}{/if}{if $product.vzor_popis}, {$product.vzor_popis}{/if}</a>
			{/if}
				...
				<ul class="cart_block_customizations" id="customization_{$productId}_{$productAttributeId}">
					{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
						<li name="customization">
							<div class="deleteCustomizableProduct" id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}_{if $product.id_kovanie}{$product.id_kovanie|intval}{else}0{/if}_{if $product.id_vzor}{$product.id_vzor|intval}{else}0{/if}"><a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$static_token}&amp;id_kovanie={$product.id_kovanie}&amp;id_vzor={$product.id_vzor}", true)}" rel="nofollow"> </a></div>
