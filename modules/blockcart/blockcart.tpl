...
<div id="cart_block" class="block exclusive">
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dt id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}_{if $product.id_kovanie}{$product.id_kovanie}{else}0{/if}_{if $product.id_vzor}{$product.id_vzor}{else}0{/if}_{if $product.id_dvere}{$product.id_dvere}{else}0{/if}_{if $product.id_zarubna}{$product.id_zarubna}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
			...
			<dd id="cart_block_combination_of_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}_{$product.id_address_delivery|intval}_{if $product.id_kovanie}{$product.id_kovanie}{else}0{/if}_{if $product.id_vzor}{$product.id_vzor}{else}0{/if}_{if $product.id_dvere}{$product.id_dvere}{else}0{/if}_{if $product.id_zarubna}{$product.id_zarubna}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<a href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)}" title="{l s='Product detail' mod='blockcart'}">{$product.attributes_small}{if $product.kovanie_popis}, {$product.kovanie_popis}{/if}{if $product.vzor_popis}, {$product.vzor_popis}{/if}{if $product.dvere_popis}, {$product.dvere_popis}{/if}{if $product.zarubna_popis}, {$product.zarubna_popis}{/if}</a>
			{/if}
				...
				<ul class="cart_block_customizations" id="customization_{$productId}_{$productAttributeId}">
					{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
						<li name="customization">
							<div class="deleteCustomizableProduct" id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}_{if $product.id_kovanie}{$product.id_kovanie|intval}{else}0{/if}_{if $product.id_vzor}{$product.id_vzor|intval}{else}0{/if}_{if $product.id_dvere}{$product.id_dvere|intval}{else}0{/if}_{if $product.id_zarubna}{$product.id_zarubna|intval}{else}0{/if}"><a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$static_token}&amp;id_kovanie={$product.id_kovanie}&amp;id_vzor={$product.id_vzor}&amp;id_dvere={$product.id_dvere}&amp;id_zarubna={$product.id_zarubna}", true)}" rel="nofollow"> </a></div>
