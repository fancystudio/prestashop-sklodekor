...
class OnePageCheckout extends Module
{
    ...
    function __construct()
    {
        ...
        $this->ps_versions_compliancy['max'] = '1.6.0';

        parent::__construct(); // The parent construct is required for translations

        $this->page        = basename(__FILE__, '.php');
        $this->displayName = ($cookie_set) ? $this->l('One Page Checkout for PS 1.5') : 'One Page Checkout for PS 1.5';
...
//        }
    ...
    function _setOptions()
    {
        ...
        $this->module_settings = array(
            "general"          => array(
                "two_column_opc"             => new Option(OptionType::RadioBox, "Two column checkout", "Displays checkout form in 2 columns. For best results, please <a onclick='setTwoColumnOptions($(this))' class='highlight_anchor'>click here</a> to set also these options: 'sample values'-off, 'sticky header cart'-off, 'cart summary at bottom'-on, 'offer password on top'-on, 'remove ref.'-on"),
                "scroll_cart"                => new Option(OptionType::RadioBox, "Sticky cart block", "Keep cart block sticky when scrolling down on checkout page."),
                "scroll_header_cart"         => new Option(OptionType::RadioBox, "Sticky header cart", "Keep header cart (e.g. ps1.5 default theme) sticky when scrolling down on checkout page."),
                ...
                "scroll_products"            => new Option(OptionType::RadioBox, "Sticky cart products", "Make also products in cart summary floating. Sticky cart summary option above must be turned on."),
                ...
                "remove_ref"                 => new Option(OptionType::RadioBox, "Remove Ref. field", "Removes Ref. field from cart-summary, to make more room for product name."),
                ...
                "animate_fields_padding"     => new Option(OptionType::RadioBox, "Animate fields padding", "Should spaces between address fields squeeze when invoice address is expanded?"),
                ...
                "display_info_block"         => new Option(OptionType::RadioBox, "Display info block", "Displays info block below cart block on checkout page. For customization, please see file /modules/onepagecheckout/views/templates/fron/info-block-content.tpl"),
                "ship2pay"                   => new Option(OptionType::RadioBox, "Ship2pay module support", 'Support for ship2pay module. For more instructions how to set it up, please see  <a href="'.__PS_BASE_URI__.'modules/onepagecheckout/Installation_and_Setup.pdf" class="highlight_anchor" target="_blank">Installation_and_Setup document</a>'),
                ...
                "override_checkout_btn"      => new Option(OptionType::RadioBox, "Override checkout button", "Override checkout button when being on checkout page. When overriden, default 'page reload' is replaced by fast scroll down to payment methods and blink of payment methods to gently notify customer."),
                "email_verify"               => new Option(OptionType::RadioBox, "Email verify box", "Displays additional email input field, so that customer enters email twice when 'registering' on OPC page."),
                "live_zip"                   => new Option(OptionType::RadioBox, "Live ZIP code", "Refresh shipping methods / prices when ZIP code changes. Necessary for ZIP based shipping modules."),
                "cart_summary_bottom"        => new Option(OptionType::RadioBox, "Cart Summary at bottom", "Must have for German e-shops to comply with new regulations from 1.8.2012"),
                "above_confirmation_message" => new Option(OptionType::RadioBox, "Message above Confirm buttom", "Display message (e.g. EU regulations / taxes) just above Confirmation button. Please translate 'above confirmation button msg' in BO / Tools / Translations / Translations of installed modules"),
                "order_detail_review"        => new Option(OptionType::RadioBox, "Summary review box", "Display box with Shipping / Invoice address / Carrier / Payment review, above confirmation button."),
                "goods_return_cms"           => new Option(OptionType::InputField, "Goods return CMS ID", "Display message 'You are entitled to cancel your order within 7 Working Days upon goods receive.' (translatable) with link to CMS for more details. Set to 0 to disable."),
                "move_cgv"                   => new Option(OptionType::RadioBox, "Move Terms & Conditions", "Moves Terms & Conditions checkbox down from carrier section, above 'I confirm my order' button."),
                "responsive_layout"          => new Option(OptionType::RadioBox, "Responsive Layout", "Responsive cart-summary table layout - NB: you need to have responsive theme to use this feature."),
                "company_based_vat"          => new Option(OptionType::RadioBox, "Company based VAT", "Force EU VAT field to behave like in standard PS, i.e. when company is filled in, it'll show up, regardless of country chosen."),
            ...
            "customer"         => array(
                ...
                "display_password_msg"   => new Option(OptionType::RadioBox, "Display create account message", "Display 'Create an account and enjoy benefits of registered customers' message above password box (applicable only when guest checkout is turned OFF in PS preferences)"),
                "password_checked"       => new Option(OptionType::RadioBox, "Create account checked", "Checkbox 'Create an account...' will be checked by default."),
                "hide_password_box"      => new Option(OptionType::RadioBox, "Hide password box", "The 'Create an account...' checkbox and password field won't be displayed at all."),
            ...
            "delivery_address" => array(
                ...
                "phone_mobile_delivery"    => new Option(OptionType::RadioBox, "Mobile Phone", "Display field 'Mobile phone' in delivery address. Applicable only when 'Phone number' in Preferences / Customers is turned off."),
                "phone_delivery"           => new Option(OptionType::RadioBox, "Home Phone", "Display field 'Home phone' in delivery address."),
                ...
                "virtual_no_delivery"      => new Option(OptionType::RadioBox, "Virtual goods - no delivery address", 'Hide all fields in delivery address when only downloadable products are in cart. <strong>Please read  <a href="'.__PS_BASE_URI__.'modules/onepagecheckout/Installation_and_Setup.pdf" class="highlight_anchor" target="_blank">Installation_and_Setup document</a> for additional changes required when this option is turned on!</strong>'),
            ),
            "invoice_address"  => array(
                "invoice_checkbox"        => new Option(OptionType::RadioBox, "Offer invoice address", "Display checkbox 'Please use another address for invoice'."),
                ...
                "phone_mobile_invoice"    => new Option(OptionType::RadioBox, "Mobile Phone", "Display field 'Mobile phone' in invoice address. Applicable only when 'Phone number' in Preferences / Customers is turned off."),
                "phone_invoice"           => new Option(OptionType::RadioBox, "Home Phone", "Display field 'Home phone' in invoice address."),
            ...
            "system"           => array(
                ...
                "before_info_element"   => new Option(OptionType::InputField, "Before InfoBlock element", "Element after which infoblock should be displayed - use this as an alternative if you don't have standard blockcart in left or right column. By default, value is '#cart_block' and info block displays right under the cart. With PS 1.5 default template (and similar), use value '#cart_block .block_content' (applicable with scroll header cart option)."),
                "update_payments_relay" => new Option(OptionType::RadioBox, "Update payments after account save", "This options is required only for relay-style payment modules, or generally any payment module handling with http_cookie, like Moneybookers, ePay, Payson or Dibs. If you use such module and use relay mode, turn this option ON."),
                "online_country_id"     => new Option(OptionType::InputField, "Online country id", 'ID of country set to be as Online delivery country for virtual items.'),
                "payment_radio_buttons" => new Option(OptionType::RadioBox, "Payment options as radio buttons", "Display payment options as radio buttons on checkout page. If turned off, payment options will be displayed in blocks (default in PS). For most payment modules radio buttons work better and rarely it needs to be turned off."),
                "cookie_cache"          => new Option(OptionType::RadioBox, "OPC form cookie cache", "Use cookie saving / restoring of fields when leaving OPC form, even when customer did not confirmed the form."),
                "save_account_overlay"  => new Option(OptionType::RadioBox, "Save account overlay", "Displays overlay on payment methods, thus requiring customer to click 'Save account' prior to proceeding with payment.")
            )
        );
    }
    ...
    public function _getAllOptionsValues()
    {
        ...
        foreach ($config_keys as $config_key) {
            ...
            $config[strtolower(substr($config_key, $prefix_length))] = $value;
        }
    ...
    private function _copyTranslations()
    {
        ...
        foreach (Language::getLanguages() as $curr_lang) // get all active languages
        {
            ...
            foreach ($translation_files as $filename) {
                $content = @file_get_contents(_PS_MODULE_DIR_ . $mod . '/views/templates/front/' . $filename);
    ...
    function install()
    {
        ...
        if (!parent::install()
            OR (_THEME_NAME_ != "default" && Configuration::updateValue(strtoupper($this->conf_prefix . 'scroll_cart'), 1) == false)
            OR (_THEME_NAME_ == "default" && Configuration::updateValue(strtoupper($this->conf_prefix . 'scroll_header_cart'), 1) == false)
            ...
            OR (_THEME_NAME_ != "default" && Configuration::updateValue(strtoupper($this->conf_prefix . 'page_fading'), 1) == false)
            ...
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'phone_mobile_delivery'), 1) == false
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'phone_mobile_invoice'), 1) == false
            ...
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'before_info_element'), (_THEME_NAME_ != "default")?'#cart_block':'#cart_block .block_content') == false
            ...
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'online_country_id'), $this->_setOnlineCountry()) == false
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'display_password_msg'), 1) == false
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'override_checkout_btn'), 1) == false
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'animate_fields_padding'), 1) == false
            OR Configuration::updateValue(strtoupper($this->conf_prefix . 'invoice_checkbox'), 1) == false
        ...
        Configuration::updateValue(strtoupper($this->conf_prefix . 'update_payments_relay'), 1);
    ...
    function getContent()
    {
        ...
        if (Tools::isSubmit('emailLangFile')) {
            $this->emailLangFile();
            Configuration::updateValue(strtoupper($this->conf_prefix . 'lang_file_sent'), 1);
...
//        }
    ...
    private function _displayEmailLangFile() {
        ...
        $ret = '';
        if (Configuration::get(strtoupper($this->conf_prefix . 'lang_file_sent')))
...
//        $divLangName     = 'text_left¤text_right';
    ...
    public function emailLangFile()
    {
        ...
        $mod_lang_file = _PS_MODULE_DIR_ . 'onepagecheckout/translations/' . $def_lang["iso_code"] . ".php";
        $content       = @file_get_contents($mod_lang_file);
    ...
    public function emailSupportRequest()
    {
        ...
        $root_length = strlen(_PS_ROOT_DIR_);
        ...
        foreach ($check_permissions as $filename) {
            ...
            $debug_info .= substr($filename, $root_length) . ": " . substr(sprintf('%o', fileperms($filename)), -4) . ", " . $owner['name'] .
