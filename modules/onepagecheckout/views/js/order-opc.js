...
function updateCarrierList(json) {
    ...
    if (typeof opc_move_cgv !== 'undefined' && jQuery.trim(opc_move_cgv) == "1") {
        ...
        $('#tos_moved').append($('#opc_delivery_methods input#cgv').parent());
    }
}
    ...
    $.ajax({
        ...
        success:function (jsonData) {
            if (jsonData.hasError) {
                ...
                // TODO: v orig order-opc.js sa vola jsonData.carrier_data, overit ci je to takto OK!
...
function setScrollHandler() {
    ...
    info_below_header_cart = (opc_before_info_element == '#cart_block .block_content' && scroll_header_cart && scroll_info);
...
function setPaymentModuleHandler() {
    ...
    var sel = $('form#paypalpro_form input#paypalpro_submit, form[name=cs_cardsave_direct_form] input#asubmit, form#stripe-payment-form .stripe-submit-button');
...
//        console.info(s1);
    ...
    $('.payment_module').not($('a#click_cs_cardsave_direct').parent()).not($('form#stripe-payment-form').parent()).unbind('click').click(_payment_module_handler);
...
// Capitalize letters as customer types for configurable fields
    ...
    if (typeof opc_move_cgv !== 'undefined' && jQuery.trim(opc_move_cgv) == "1") {
        $('<div id="tos_moved"></div>').insertBefore('form#offer_password');
    }
