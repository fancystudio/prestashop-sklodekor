        ...
        <tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}"
            ...
            <td class="price_discount_del">
                {if strlen($discount.code)}<a
                        ...
                        class="price_discount_delete" title="{l s='Delete' mod='onepagecheckout'}">{l s='Delete' mod='onepagecheckout'}</a>{/if}
            </td>
        </tr>
        {/foreach}
    </tbody>
...
{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
    <tr class="cart_total_delivery">
        ...
        <td colspan="2" class="price" id="total_shipping">{l s='Free Shipping!' mod='onepagecheckout'}</td>
    </tr>
{else}
    {if $use_taxes}
        {if $priceDisplay}
...
{/if}
<tr class="cart_total_price">
    <td colspan="{$colspan}" id="cart_voucher" class="cart_voucher">
        {if $voucherAllowed}
            ...
            <form action="{if $opc}{$link->getPageLink('order-opc.php', true)}{else}{$link->getPageLink('order.php', true)}{/if}"
                ...
                <fieldset>
                    ...
                    <p class="submit"><input type="hidden" name="submitDiscount"/><input type="submit"
                        ...
                        <div id="display_cart_vouchers">
                            {foreach $displayVouchers as $voucher}
                                ...
                                <span onclick="$('#discount_name').val('{$voucher.code}');return false;"
                                      class="voucher_name">{$voucher.code}</span> - {/if}{$voucher.name}<br />
                            {/foreach}
                        </div>
                    {/if}
                </fieldset>
    ...
    {if $use_taxes}
        <td colspan="2" class="price total_price_container" id="total_price_container">
            <p>{l s='Total:' mod='onepagecheckout'}</p>
...
<div id="HOOK_SHOPPING_CART">{$HOOK_SHOPPING_CART}</div>
    ...
    <p class="cart_navigation">
        <b>
            ...
            <a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, '{$opckt_script_name}')) || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index.php')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}"
