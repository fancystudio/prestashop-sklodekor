...
//                            break;
    ...
    public function setMedia()
    {
        ...
        // opckt: Adding CSS style sheet - mostly empty, customization possible
        $this->addCSS(_MODULE_DIR_ . 'onepagecheckout/views/css/order-opc.css');
        // opckt: 2-column OPC checkout stylesheet
        if ($this->opc_config['two_column_opc'] > 0)
            $this->addCSS(_MODULE_DIR_ . 'onepagecheckout/views/css/two-column-opc.css');
        if ($this->opc_config['responsive_layout'] > 0)
            $this->addCSS(_MODULE_DIR_ . 'onepagecheckout/views/css/responsive.css');
        // opckt: Theme's customized CSS (if exists)
        if (file_exists(_PS_MODULE_DIR_ . 'onepagecheckout/views/css/' . _THEME_NAME_ . '.css'))
            $this->addCSS(_MODULE_DIR_ . 'onepagecheckout/views/css/' . _THEME_NAME_ . '.css');

        // opckt: Adding JS files
        $this->addJS(_MODULE_DIR_ . 'onepagecheckout/views/js/jquery.cookie.js');
    ...
    public function initContent()
    {
        ...
        if ($upsell && $upsell->active && !(Tools::getValue('skip_offers') == 1 || $internal_referrer)) {
            ...
            if (Tools::isSubmit('cart-only')) {
                ...
                $this->context->smarty->assign('order_process_type', Configuration::get('PS_ORDER_PROCESS_TYPE'));
                $this->setTemplate($this->opc_templates_path . '/shopping-cart.tpl');
            } else {
                $this->setTemplate($this->opc_templates_path . '/order-opc.tpl');
            }
        }
    }
    ...
    protected function _getGuestInformations()
    {
        ...
        if (((int)$this->context->cart->id_address_invoice != (int)$this->context->cart->id_address_delivery)) {
    ...
    protected function _getCarrierList()
    {
        ...
        elseif ((!Validate::isLoadedObject($address_delivery) || $address_delivery->deleted) && $this->context->cart->id_address_delivery != 0)
            ...
            $result = array(
                ...
                'carrier_block'      => $this->context->smarty->fetch($this->opc_templates_path . '/order-carrier.tpl')
            );
